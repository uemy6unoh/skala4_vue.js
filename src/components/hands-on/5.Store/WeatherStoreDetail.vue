...
const weatherList = [
  { id: 'city_01', name: '서울', area: '서울특별시', temp: 28, status: '맑음', humidity: 62, wind: 2.8, fineDust: 92 },
  { id: 'city_02', name: '안양', area: '경기도 안양시', temp: 27, status: '맑음', humidity: 58, wind: 3.1, fineDust: 42 },
  { id: 'city_03', name: '천안', area: '충청남도 천안시', temp: 23, status: '흐림', humidity: 76, wind: 4.5, fineDust: 68 },
  { id: 'city_04', name: '거제', area: '경상남도 거제시', temp: 25, status: '비', humidity: 88, wind: 9.3, fineDust: 22 },
  { id: 'city_05', name: '경주', area: '경상북도 경주시', temp: 29, status: '맑음', humidity: 81, wind: 6.2, fineDust: 28 },
]

const selectedCity = computed(() => {
  return weatherList.find((city) => city.id === route.params.cityId)
})
...
<template>
  <section class="page-card">
    <template v-if="selectedCity">
      ...
      >
        {{ favoriteStore.isFavorite(selectedCity.id) ? '관심 도시 해제' : '관심 도시 추가' }}
      </button>
    </template>
    ...
    <RouterLink :to="{ name: 'store-home' }" class="back-link">
      Store 대시보드로 돌아가기
    </RouterLink>
  </section>
</template>
