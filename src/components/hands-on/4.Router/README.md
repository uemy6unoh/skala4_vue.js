...
### 날씨 필터를 바꿀 때 카드 위치가 움직이는 문제

전체 목록에서 흐림이나 비로 필터를 바꾸면 카드 수가 줄면서 세로 스크롤바가 사라질 수 있었습니다. 이때 화면의 사용 가능한 너비가 달라져 카드가 옆으로 조금 움직여 보였습니다. main.css에서 스크롤바 영역을 항상 확보해 필터를 변경해도 카드의 가로 위치가 유지되도록 했습니다.
